...
<body style="background-color: rgb(213, 249, 255);">
    ...
    <section class="w3l-skill-breadcrum">
        <div class="breadcrum" style="background-color: rgb(213, 249, 255);"><br>
            <div class="container" style="background-color: rgb(200, 246, 255);">
                <p><a href="index.php">Home</a> &nbsp; / &nbsp; Student SignUp</p>
            </div>
        </div>
    </section>
    <!-- //breadcrum -->
    ...
    <div class="hotel-bothside">
		<div class="header-section text-center"><br>
            <h3 class="mb-md-5 mb-4">Student Sign-Up</h3>
        </div>
...
<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        if($_FILES['fileImg']['size'] == 0 && $_FILES['fileImg']['error'] == 0)
        {
            ...
            $sql = "insert into `student`(`fname`,`lname`,`gender`,`dob`,`address`,`mobile`,`email`,`password`,`img_name`,`img_path`) values ('".$fname."','".$lname."','".$gender."','".$dob."','".$address."','".$mobileno."','".$email."','".$passcode."','".$image_name."','".$file_path."');";
        ...
        else
        {
            ...
            {  
                ...
                {
                    $ret=$_SERVER['HTTP_REFERER'];
                    echo "<script type='text/javascript'>alert('Sorry! $file_name image is already exist. Please rename your image then try again.');
                    window.location='$ret'</script>";
                }
                else{
                    $sql = "insert into `student`(`fname`,`lname`,`gender`,`dob`,`address`,`mobile`,`email`,`password`,`img_name`,`img_path`) values ('".$fname."','".$lname."','".$gender."','".$dob."','".$address."','".$mobileno."','".$email."','".$passcode."','".$image_name."','".$file_path."');";
        ...
        if (!$query) {
            ...
            $sql0 = "insert into `subscribers`(`email`) values ('" . $email . "');";
            $query = mysqli_query($conn, $sql0);
            $sub = "Welcome to MyGuru Onlne Course Portal";
            $headers = "From: MyGuru\-online ";
            $body = "Hi ".$fname." ".$lname.",
            \nCongratualation. You have been successfully registered in the MyGuru Online Course Portal as Student.
            \nYour Login credentials for the portal are as below
            \nLogin ID : ".$email."\nPassword : ".$passcode."
            \n\nPlease login into the website today itself to start Learning. Best of luck for your eLearning.
            \nRegards,\nSupport Team
